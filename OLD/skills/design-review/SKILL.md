...
Version: v29 (2026-08-19). When starting an audit, silently note this version internally; if the user asks which version is installed, tell them.
...
1. **Overall impression** - the score card widget plus one JTBD-anchored line, nothing else.
   ...
   **Score card** (rendered inline with the Visualizer as a flat HTML card, adapting to light/dark via CSS variables - never hardcoded colors):
   ...
   - Footer row: severity count badges - Blocker, High, Medium, Low/Nit - each showing its count, color-coded (red, orange-red, amber, gray). Omit zero-count badges.
