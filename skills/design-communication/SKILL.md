...
Structure: the decision, the user need and evidence behind it, options considered with why-rejected (2-3, honestly represented), tradeoffs accepted, and what would change our mind. Connect to `design-strategy` and connected analytics where they exist; label judgment as judgment - laundering taste as data destroys credibility exactly once.
...
- File output: use an available organization-brand presentation skill when the user requests a deck; otherwise provide a structured outline.
...
Translate design work into outcome language: metric moved (with baseline and attribution honesty), cost avoided (support tickets, engineering rework), velocity gained (system adoption, reuse), and risk reduced (accessibility compliance, churn signals addressed). Report the chain: design change -> behavior change -> business number. Where attribution is shared, say "contributed to". Pull numbers from connected evidence; never invent.
...
Inline drafts and outlines by default; formatted documents or decks only on request and through an available organization-brand workflow. For a difficult stakeholder, tailor the evidence, decision, tradeoffs, and ask to that audience without inventing motives.
