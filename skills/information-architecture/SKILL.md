...
description: >
  ...
  where-does-it-go or what-do-we-call-the-section question. Do not use for isolated interface-string
  rewrites when the structure is already decided; route those to ux-writing.
metadata:
  version: "0.1.0"
  phase: ideate
---
...
1. **Inventory.** List the content/features being organized, with task frequency from connected analytics when instrumented and audience per item.
...
3. **Structure.** Depth vs breadth: prefer broader-and-shallower; minimize unnecessary levels and justify deep paths. Every level must pass the trigger-word test: would the user's own vocabulary lead them here? Validate contested structures with a card sort or tree test through `design-validation`.
...
Inline: proposed structure as an indented tree with per-node label rationale, the organizing principle and rejected alternatives, migration notes if restructuring, and open questions with validation plan. Route flow-level questions to `flows-and-states` and visible navigation design or implementation to `frontend-design`.
