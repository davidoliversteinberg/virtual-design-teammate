...
4. **Identify design risks** and what would de-risk each: judgment, a technical spike, connected evidence, or a focused plan through `design-validation`.
...
Inline: mode stated, then the plan or brief, then numbered open questions. Create a formatted document only when requested and an appropriate document workflow is available. Route strategy-level direction to `design-strategy` and journeys to `journey-mapping`.
