...
const REMOTE_MANIFEST_URL =
  "https://raw.githubusercontent.com/davidoliversteinberg/virtual-design-teammate/main/.claude-plugin/plugin.json";
...
const cacheDirectory =
  ...
  path.join(os.homedir(), ".cache", "virtual-design-teammate");
...
function parseVersion(source) {
  ...
  if (!match) throw new Error("Could not read the installed frontend-design version.");
  return match[1];
}
...
function emit(result) {
  ...
  if (result.status === "update-available") {
    process.stdout.write(
      `Virtual Design Teammate update available: ${result.localVersion} -> ${result.remoteVersion}. Notify the user; do not update automatically.\n`,
    );
  } else if (result.status === "up-to-date") {
    process.stdout.write(`Virtual Design Teammate is up to date (${result.localVersion}).\n`);
  } else if (result.status === "local-newer") {
    process.stdout.write(
      `Installed Virtual Design Teammate (${result.localVersion}) is newer than published main (${result.remoteVersion}).\n`,
    );
  } else {
    process.stdout.write(
      "Virtual Design Teammate update check unavailable; continuing without blocking the task.\n",
    );
  }
}
...
try {
  ...
  if (!remoteVersion) {
    const response = await fetch(REMOTE_MANIFEST_URL, {
      headers: { "user-agent": "virtual-design-teammate-update-check" },
    });
