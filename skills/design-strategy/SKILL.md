...
description: >
  ...
  "reward effort scorecard", "what should we do first", or needs to connect design choices to business
  outcomes and H1/H2 planning. Trigger for any which-bet-and-why conversation. Do not use when the job is
  defining a KPI, baseline, target, or guardrail; route that to success-metrics.
metadata:
  version: "0.1.0"
  phase: define
---
...
- Dimensions: user impact (severity x reach, evidenced), business impact (revenue, retention, or cost from connected evidence), effort (design plus engineering, with an engineering check), and confidence in the evidence.
...
Inline and decision-ready by default. For executive circulation, use `design-communication` to structure the narrative and an available organization-brand document or presentation workflow when the user requests a file.
