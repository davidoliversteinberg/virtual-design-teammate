...
Inline: flow diagram, state table per screen, and a risk list (states with unresolved design). Route micro-level behavior to `interaction-patterns`, visible layout and implementation to `frontend-design`; formal state-machine modeling for complex components stays here: states, events, transitions, and illegal-transition guards in a table.
