...
description: >
  ...
  controls, or media. Do not use as the lead for a broad multi-lens design critique; route that to
  design-review and load this skill for WCAG depth.
...
Design-time: requirements woven into the spec, marked "a11y:". Audit: findings table + summary. Route visible implementation and rendered verification to `frontend-design`, and copy fixes to `ux-writing`. Note honestly what a static review cannot verify (actual screen-reader behavior, reflow, or device interaction) and list it for engineering testing.
