...
**Goal-gradient effect.** Motivation accelerates near a goal. Apply: show progress with the end visible, seed initial progress only when it is truthful, and break long journeys into meaningful stages. Limits: fake progress bars and inflated steps are dark patterns; flag them in `design-review`.
